...
class DashboardAPIController extends AppBaseController
{
    ...
    public function getSalesItemsByShop($shopName)
    {
        ...
        $saleItems = SaleItem::whereHas('sale', function ($query) use ($shop) {
            $query->where('shop_id', $shop->id);
        })->with('product', 'sale')->get()->map(function ($item) {
            return [
        ...
        return $this->sendResponse($saleItems, 'Sale items retrieved successfully');
    }
