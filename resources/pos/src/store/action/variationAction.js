...
export const fetchAllVariations = () => async (dispatch) => {
    try {
        const response = await apiConfig.get(apiBaseURL.VARIATIONS + `?page[size]=0`);
        dispatch({
            type: variationActionType.FETCH_ALL_VARIATIONS,
            payload: response.data.data,  // Ensure this is the correct format
        });
    } catch (error) {
        console.error("Error fetching variations:", error);  // Log error
        dispatch(
            addToast({
                text: error.response?.data?.message || 'Failed to fetch variations',
                type: toastType.ERROR,
            })
        );
    }
