...
export const allSalesForShopByProdCategory = (shopId) => async (dispatch) => {
    dispatch(setLoading(true));
    await apiConfig
        .get(`${apiBaseURL.SALES}/shop/${shopId}/by-product-category`)
        .then((response) => {
            dispatch({
                type: saleActionType.GET_SALES_BY_SHOP_CATEGORY,
                payload: response.data.data,
            });
            dispatch(setLoading(false));
        })
        .catch(({ response }) => {
            ...
            dispatch(
                addToast({
                    text: response?.data?.message || "Failed to fetch sales by category",
                    type: toastType.ERROR,
                })
            );
        });
